...
template<typename T> DynamicArray<T>::DynamicArray(): size(0) {this->size = 0;}

template<typename T> DynamicArray<T>::DynamicArray(int size) : size(size){
    if (size<0) {throw out_of_range("index_oor");}
    this->size = size;
...
template<typename T> DynamicArray<T>::DynamicArray(T* items, int size) : DynamicArray(size) {
    if (size<0) {throw out_of_range("index_oor");}
    this->size =size;
    for (int i = 0; i < size; i++) {this->data[i] = items[i];}
}

template<typename T> DynamicArray<T>::DynamicArray(const DynamicArray<T> &array) : DynamicArray(array.size) {
    if (size > array.size || size<0) {throw out_of_range("index_oor");}
    this->size = size;
    for (int i = 0; i < array.size; i++){this->data[i] = array.data[i];}
}

template<typename T> T DynamicArray<T>::Get(int index) const {
    if (index < 0 || index >= this->size) {throw out_of_range("index_oor");}
